...
flowchart TD
    ...
    O --> P[✅ บันทึกสำเร็จ → ส่ง Confirm Message]
...
flowchart TD
    ...
    B --> J[📚 การศึกษา]
    B --> K[💰 เงินเดือน (รายรับ)]
    B --> L[🎁 รายได้อื่นๆ (รายรับ)]

    A --> M[➕ สร้างหมวดใหม่]
    M --> N[ใส่ชื่อ + เลือก Icon + เลือกสี]
    N --> O[เลือก type: รายรับ/รายจ่าย]
    O --> P[บันทึก]
```
...
flowchart TD
    ...
    H -->|ไม่ใช่| J[ตอบ: ป้าไม่เข้าใจ ลองพิมพ์ใหม่นะ]

    D -->|Image| K{หลานโปร?}
    K -->|ใช่| L[AI อ่านใบเสร็จ/สลิป]
    K -->|ไม่| M[ตอบ: ฟีเจอร์นี้สำหรับหลานโปร]

    D -->|Audio| N{หลานโปร?}
    N -->|ใช่| O[Speech-to-text → บันทึก]
    N -->|ไม่| M

    G --> P[Reply: บันทึกสำเร็จ ✅]
```
...
```mermaid
flowchart LR
    A[⏰ Cron: ทุกวันที่ 1 เวลา 08:00] --> B[ดึงข้อมูลเดือนที่แล้ว]
...
```mermaid
flowchart LR
    A[⏰ Cron: ทุกวันจันทร์ 09:00] --> B[ดึงข้อมูล 7 วันที่ผ่านมา]
...
```mermaid
flowchart LR
    A[⏰ Cron: ทุกวัน 20:00] --> B{วันนี้ user จดหรือยัง?}
    B -->|ยัง| C[ส่ง LINE: อย่าลืมจดนะหลาน 📝]
...
```mermaid
flowchart LR
    A[⏰ Cron: ทุกวัน 00:01] --> B[ค้นหา recurring ที่ถึงกำหนด]
    B --> C[สร้าง transaction อัตโนมัติ]
    C --> D[ส่ง LINE แจ้ง: บันทึกค่าเน็ต 599 อัตโนมัติ ✅]
```
...
flowchart TD
    ...
    S2 --> S2a[Filter: วันที่ / หมวด / ประเภท]
    S2 --> S2b[Search]
    S2 --> S2c[แก้ไข / ลบ]

    NAV3 --> S3[Add Transaction Form]
    S3 --> S3a[จำนวนเงิน]
    S3 --> S3b[ประเภท: รายรับ/รายจ่าย]
